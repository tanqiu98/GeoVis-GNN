...
def load_mphoi_72_training_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr, model_name: str, 
    ...
    if debug:
        training_data = training_data[:4]
        val_data = val_data[:1]
    train_loader, scalers, _ = create_data_loader(training_data, model_name, model_input_type, 'mphoi72',
                                                  batch_size=batch_size, shuffle=True,
                                                  scaling_strategy=scaling_strategy, sigma=sigma,
                                                  downsampling=downsampling, test_data=False)
    val_loader, _, _ = create_data_loader(val_data, model_name, model_input_type, 'mphoi72', batch_size=len(val_data),
                                          shuffle=False, scalers=scalers, sigma=sigma, downsampling=downsampling,
                                          test_data=False)
    input_size = input_size_from_data_loader(train_loader, model_name, model_input_type)
    data_info = {'input_size': input_size}
...
def load_mphoi_120_training_data(data_path, data_path_zarr, data_path_obbs_zarr,  data_path_hbbs_zarr, data_path_hps_zarr, model_name: str, 
                             model_input_type: str, test_subject_id: int, batch_size: int = 8, val_fraction: float = 0.2, seed: int = 42, 
                             debug: bool = False, scaling_strategy=None, sigma: float = 0.0, downsampling: int = 1):
    with open(data_path, mode='rb') as f:
        data = json.load(f)
    ...
    for video_id, human_ground_truth in data.items():
        subject_id, task, take = video_id.split(sep='-')
        first_sub, second_sub, third_sub = int(subject_id[-3]), int(subject_id[-2]), int(subject_id[-1])
        test_subject_set = set([int(test_subject_id[-3]), int(test_subject_id[-2]), int(test_subject_id[-1])])
        if (first_sub in test_subject_set) or (second_sub in test_subject_set) or (third_sub in test_subject_set):
            continue
        human_visual_features = np.load(os.path.join(data_path_zarr, "humans", "{}.npy".format(video_id)))
        orig_object_visual_features_obj = np.load(os.path.join(data_path_zarr, "objects", "{}.npz".format(video_id)))
        object_visual_features = orig_object_visual_features_obj["visual_feature"]
        object_keypoints = np.load(os.path.join(data_path_obbs_zarr, "{}.npy".format(video_id)))
        human_bounding_boxes = np.load(os.path.join(data_path_hbbs_zarr, "{}.npy".format(video_id)))
        human_joints = np.load(os.path.join(data_path_hps_zarr, "{}.npy".format(video_id)))
        
        Human1_features = human_visual_features[:, 0, :]
        Human2_features = human_visual_features[:, 1, :]
        Human3_features = human_visual_features[:, 2, :]
        object_features = object_visual_features
        no_frames = len(object_keypoints)
        object_kpts = object_keypoints.reshape(no_frames, 5, 4)
        Human1_bbs = human_bounding_boxes[:, 0, :]
        Human2_bbs = human_bounding_boxes[:, 1, :]
        Human3_bbs = human_bounding_boxes[:, 2, :]
        Human1_hps = human_joints[:, 0, :]
        Human2_hps = human_joints[:, 1, :]
        Human3_hps = human_joints[:, 2, :]
        training_data.append([Human1_features, Human2_features, Human3_features, object_features, human_ground_truth,
                              Human1_bbs, Human2_bbs, Human3_bbs, object_kpts, Human1_hps, Human2_hps, Human3_hps])
    training_data, val_data = split_train_test(training_data, test_fraction=val_fraction, seed=seed)
    if debug:
        training_data = training_data[:4]
        val_data = val_data[:1]
    train_loader, scalers, _ = create_data_loader(training_data, model_name, model_input_type, 'mphoi120',
                                                  batch_size=batch_size, shuffle=True,
                                                  scaling_strategy=scaling_strategy, sigma=sigma,
                                                  downsampling=downsampling, test_data=False)
    val_loader, _, _ = create_data_loader(val_data, model_name, model_input_type, 'mphoi120', batch_size=len(val_data),
                                          shuffle=False, scalers=scalers, sigma=sigma, downsampling=downsampling,
                                          test_data=False)
    input_size = input_size_from_data_loader(train_loader, model_name, model_input_type)
    data_info = {'input_size': input_size}
...
def load_training_data(data, model_name, model_input_type, batch_size: int = 8, val_fraction: float = 0.2,
    ...
    elif 'MPHOI_72' in data_path:
        data_path_obbs_zarr = data.path_obb_zarr
        data_path_hbbs_zarr = data.path_hbb_zarr
        data_path_hps_zarr = data.path_hps_zarr
        train_loader, val_loader, data_info, scalers = \
            load_mphoi_72_training_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr, 
    ...
    elif 'MPHOI_120' in data_path:
        data_path_obbs_zarr = data.path_obb_zarr
        data_path_hbbs_zarr = data.path_hbb_zarr
        data_path_hps_zarr = data.path_hps_zarr
        train_loader, val_loader, data_info, scalers = \
            load_mphoi_120_training_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr, 
                                        model_name, model_input_type, test_subject_id=test_subject_id, batch_size=batch_size, 
                                        val_fraction=val_fraction, seed=seed, debug=debug, scaling_strategy=scaling_strategy, 
                                        sigma=sigma, downsampling=downsampling)
    else:  # CAD-120
...
def load_mphoi_72_testing_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr,
    ...
    for video_id, human_ground_truth in data.items():
        ...
        testing_data.append([Human1_features, Human2_features, object_features, human_ground_truth,
                            Human1_bbs, Human2_bbs, objects_bbs, Human1_hps, Human2_hps])
        test_ids.append(video_id)
    test_loader, _, segmentations = create_data_loader(testing_data, model_name, model_input_type, 'mphoi72',
                                                       batch_size=batch_size, shuffle=False, scalers=scalers,
                                                       downsampling=downsampling, test_data=True)
    input_size = input_size_from_data_loader(test_loader, model_name, model_input_type)
    data_info = {'input_size': input_size}
...
def load_mphoi_120_testing_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr, model_name: str, 
                                model_input_type: str, test_subject_id: int, batch_size: int = 8, 
                                scalers: Optional[dict] = None, downsampling: int = 1):
    with open(data_path, mode='rb') as f:
        data = json.load(f)
    testing_data, test_ids = [], []
    for video_id, human_ground_truth in data.items():
        ...
        Human2_hps = human_joints[:, 1, :]
        Human3_hps = human_joints[:, 2, :]
        testing_data.append([Human1_features, Human2_features, Human3_features, object_features, human_ground_truth,
                             Human1_bbs, Human2_bbs, Human3_bbs, object_kpts, Human1_hps, Human2_hps, Human3_hps])
        test_ids.append(video_id)
    test_loader, _, segmentations = create_data_loader(testing_data, model_name, model_input_type, 'mphoi120',
                                                       batch_size=batch_size, shuffle=False, scalers=scalers,
                                                       downsampling=downsampling, test_data=True)
    input_size = input_size_from_data_loader(test_loader, model_name, model_input_type)
    data_info = {'input_size': input_size}
...
def load_testing_data(data, model_name: str, model_input_type: str, batch_size: int, scalers: Optional[dict] = None):
    ...
    if 'BimanualActions' in data_path:
        ...
        test_loader, data_info, segmentations, test_ids = \
            load_mphoi_72_testing_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr,
                                    model_name, model_input_type, test_subject_id=test_subject_id, batch_size=batch_size, 
        ...
        data_path_hps_zarr = data.get('path_hps_zarr', default_value=None)
        if data_path_obbs_zarr is None:
            data_path_obbs_zarr = os.path.join(os.path.dirname(data_path_zarr), 'object_keypoints/bbox_corner')
        if data_path_hbbs_zarr is None:
            data_path_hbbs_zarr = os.path.join(os.path.dirname(data_path_zarr), 'human_bounding_boxes')
        if data_path_hps_zarr is None:
            data_path_hps_zarr = os.path.join(os.path.dirname(data_path_zarr), 'human_joints')
        test_loader, data_info, segmentations, test_ids = \
            load_mphoi_120_testing_data(data_path, data_path_zarr, data_path_obbs_zarr, data_path_hbbs_zarr, data_path_hps_zarr,
                                    model_name, model_input_type, test_subject_id=test_subject_id, batch_size=batch_size, 
                                    scalers=scalers, downsampling=downsampling)
    else:  # CAD-120
        video_id_to_subject_id = read_dictionary(data.video_id_to_subject_id)
...
def create_data_loader(data, model_name: str, model_input_type: str, dataset_name: str, batch_size: int, shuffle: bool,
    ...
    if dataset_name.lower() == 'cad120':
        x, y = assemble_tensors(data, model_name, model_input_type, sigma=sigma, downsampling=downsampling,
                                test_data=test_data)
    elif dataset_name.lower() == 'mphoi72':
        x, y = assemble_mphoi_72_tensors(data, model_name, sigma=sigma, downsampling=downsampling, test_data=test_data)
    elif dataset_name.lower() == 'mphoi120':
        x, y = assemble_mphoi_120_tensors(data, model_name, sigma=sigma, downsampling=downsampling, test_data=test_data)
    else:
...
def assemble_mphoi_72_tensors(data: list, model_name: str, sigma: float = 0.0, downsampling: int = 1,
                              test_data: bool = False):
    xs, ys = assemble_mphoi_72_frame_level_recurrent_human(data, downsampling=downsampling, test_data=test_data)
    xs_objects = assemble_mphoi_72_frame_level_recurrent_objects(data, downsampling=downsampling)
    if model_name == 'GeoVisGNN':
        if sigma:
            ys[2] = ignore_last_step_end_flag_general(ys[2])
        ys[2] = smooth_segmentation(ys[2], sigma)
        ys_budget = ys[2]
        xs_hh_dists = assemble_mphoi_72_human_human_distances(data, downsampling=downsampling)
        xs_ho_dists = assemble_mphoi_72_human_object_distances(data, downsampling=downsampling)
        xs_oo_dists = assemble_mphoi_72_object_object_distances(data, downsampling=downsampling)
        xs_steps = assemble_num_steps(data, downsampling=downsampling)
        xs = xs[:1] + xs_objects + xs[1:] + [xs_hh_dists, xs_ho_dists, xs_oo_dists, xs_steps]
        ys = [ys_budget] + ys[2:] + ys[:2]
        ys += ys[-2:]
    else:
        raise ValueError(f'MPHOI_72 code not implemented for {model_name} yet.')
    return xs, ys


def assemble_mphoi_120_tensors(data: list, model_name: str, sigma: float = 0.0, downsampling: int = 1,
                              test_data: bool = False):
    xs, ys = assemble_mphoi_120_frame_level_recurrent_human(data, downsampling=downsampling, test_data=test_data)
    xs_objects = assemble_mphoi_120_frame_level_recurrent_objects(data, downsampling=downsampling)
    if model_name == 'GeoVisGNN':
        if sigma:
            ys[2] = ignore_last_step_end_flag_general(ys[2])
        ys[2] = smooth_segmentation(ys[2], sigma)
        ys_budget = ys[2]
        xs_hh_dists = assemble_mphoi_120_human_human_distances(data, downsampling=downsampling)
        xs_ho_dists = assemble_mphoi_120_human_object_distances(data, downsampling=downsampling)
        xs_oo_dists = assemble_mphoi_120_object_object_distances(data, downsampling=downsampling)
        xs_steps = assemble_num_steps(data, downsampling=downsampling)
        xs = xs[:1] + xs_objects + xs[1:] + [xs_hh_dists, xs_ho_dists, xs_oo_dists, xs_steps]
        ys = [ys_budget] + ys[2:] + ys[:2]
        ys += ys[-2:]
    else:
        raise ValueError(f'MPHOI_120 code not implemented for {model_name} yet.')
    return xs, ys
...
def assemble_mphoi_72_frame_level_recurrent_human(data, downsampling: int = 1, test_data: bool = False):
    # Input
    xs_h1, xs_h2 = [], []
    xs_h1p, xs_h2p = [], []
    x_obb= []
...
def assemble_mphoi_120_frame_level_recurrent_human(data, downsampling: int = 1, test_data: bool = False):
    # Input
    xs_h1, xs_h2, xs_h3 = [], [], []
    xs_h1p, xs_h2p, xs_h3p = [], [], []
    x_obb= []
    max_len, max_len_downsampled = 0, 0
    for Human1, Human2, Human3, _, _, _, _, _, objects_bounding_box, Human1_pose, Human2_pose, Human3_pose in data:
        # human feature
        max_len = max(max_len, Human1.shape[0])
        Human1 = Human1[downsampling - 1::downsampling]
        Human2 = Human2[downsampling - 1::downsampling]
        Human3 = Human3[downsampling - 1::downsampling]
        max_len_downsampled = max(max_len_downsampled, Human1.shape[0])
        xs_h1.append(Human1)
        xs_h2.append(Human2)
        xs_h3.append(Human3)
        # human pose
        Human1_pose = Human1_pose[downsampling - 1::downsampling]/100
        Human2_pose = Human2_pose[downsampling - 1::downsampling]/100
        Human3_pose = Human3_pose[downsampling - 1::downsampling]/100
        xs_h1p.append(Human1_pose)
        xs_h2p.append(Human2_pose)
        xs_h3p.append(Human3_pose)
        # objects bounding box
        objects_bounding_box = objects_bounding_box[downsampling - 1::downsampling]/100
        x_obb.append(objects_bounding_box)
    # xs_obb: reshape the object bounding box
    ...
    for i, video in enumerate(x_obb):
        ...
        bbb.append(bb)
    xs_obb = bbb
    # add context features to xs_h1, xs_h2, xs_h3
    hhh1 = list()
    hhh2 = list()
    hhh3 = list()
    keypoints = [1, 2, 4, 6, 7, 11, 13, 14, 27] # upper body keypoints
    for i, video in enumerate(xs_h1):
        ...
        for j, frame in enumerate(video):
            h1p = xs_h1p[i][j][keypoints]     # Human1 pose  (9*2)
            h2p = xs_h2p[i][j][keypoints]     # Human2 pose  (9*2)
            h3p = xs_h3p[i][j][keypoints]     # Human3 pose  (9*2)
            obb = xs_obb[i][j]                # object bounding box (10*2)
            # velocity
            if j+1 < len(video):
                next_h1p = xs_h1p[i][j+1][keypoints]
                h1p_velo = (next_h1p - h1p)*10
                next_h2p = xs_h2p[i][j+1][keypoints]
                h2p_velo = (next_h2p - h2p)*10
                next_h3p = xs_h3p[i][j+1][keypoints]
                h3p_velo = (next_h3p - h3p)*10
                next_obb = xs_obb[i][j+1]
                obb_velo = (next_obb - obb)*10
            else:
                h1p_velo = np.zeros((9, 2))
                h2p_velo = np.zeros((9, 2))
                h3p_velo = np.zeros((9, 2))
                obb_velo = np.zeros((10, 2))
            h1pvelo = np.hstack((h1p, h1p_velo)) # 9*4
            h2pvelo = np.hstack((h2p, h2p_velo)) # 9*4
            h3pvelo = np.hstack((h3p, h3p_velo)) # 9*4
            obbvelo = np.hstack((obb, obb_velo)) # 10*4
            h1pv = h1pvelo.reshape(1, -1)
            h2pv = h2pvelo.reshape(1, -1)
            h3pv = h3pvelo.reshape(1, -1)
            obv = obbvelo.reshape(1, -1)
            # concatenate all fetures
            args = (h1pv[0], h2pv[0], h3pv[0], obv[0])
            context = np.concatenate(args) # 148
            h1_con = np.concatenate((frame, context)) # 2196
            h2_con = np.concatenate((xs_h2[i][j], context)) # 2196
            h3_con = np.concatenate((xs_h3[i][j], context)) # 2196
            hh1.append(h1_con)
            hh2.append(h2_con)
            hh3.append(h3_con)
        hh1 = np.array(hh1)
    ...
    xs_h3 = hhh3
    feature_size = xs_h1[0].shape[-1]
    x_hs = np.full([len(xs_h1), max_len_downsampled, 3, feature_size], fill_value=np.nan, dtype=np.float32)
    for m, (h1, h2, h3) in enumerate(zip(xs_h1, xs_h2, xs_h3)):
        x_hs[m, :len(h1), 0] = h1
        x_hs[m, :len(h2), 1] = h2
        x_hs[m, :len(h3), 2] = h3
    xs = [x_hs]
    # Output
    y_rec_hs = np.full([len(x_hs), max_len, 3], fill_value=-1, dtype=np.int64)
    y_pred_hs = np.full_like(y_rec_hs, fill_value=-1)
    for m, (_, _, _, _, video_humans_ground_truth, _, _, _, _, _, _, _) in enumerate(data):
        # Human1
        y_h1 = video_humans_ground_truth['Human1']
        y_rec_hs[m, :len(y_h1), 0] = y_h1
        rle = list(run_length_encoding(y_h1))
        y_h1_p = []
        for (_, previous_length), (next_label, _) in zip(rle[:-1], rle[1:]):
        ...
        y_pred_hs[m, :len(y_h1_p), 0] = y_h1_p
        # Human2
        y_h2 = video_humans_ground_truth['Human2']
        y_rec_hs[m, :len(y_h2), 1] = y_h2
        rle = list(run_length_encoding(y_h2))
        y_h2_p = []
        for (_, previous_length), (next_label, _) in zip(rle[:-1], rle[1:]):
        ...
        y_pred_hs[m, :len(y_h2_p), 1] = y_h2_p
        # Human3
        y_h3 = video_humans_ground_truth['Human3']
        y_rec_hs[m, :len(y_h3), 2] = y_h3
        rle = list(run_length_encoding(y_h3))
        y_h3_p = []
        for (_, previous_length), (next_label, _) in zip(rle[:-1], rle[1:]):
            y_h3_p += [next_label] * previous_length
        y_pred_hs[m, :len(y_h3_p), 2] = y_h3_p
    x_hs_segmentation = segmentation_from_output_class(y_rec_hs[:, downsampling - 1::downsampling],
                                                       segmentation_type='input')
    xs.append(x_hs_segmentation)
    if not test_data:
...
def assemble_mphoi_72_frame_level_recurrent_objects(data, downsampling: int = 1):
    xs_objects = []
    max_len, max_len_downsampled, max_num_objects = 0, 0, 0
    for _, _, objects, _, _, _, _, _, _ in data:
        max_len = max(max_len, objects.shape[0])
...
def assemble_mphoi_120_frame_level_recurrent_objects(data, downsampling: int = 1):
    xs_objects = []
    max_len, max_len_downsampled, max_num_objects = 0, 0, 0
    for _, _, _, objects, _, _, _, _, _, _, _, _ in data:
        max_len = max(max_len, objects.shape[0])
        max_num_objects = max(max_num_objects, objects.shape[1])
        objects = objects[downsampling - 1::downsampling]
        max_len_downsampled = max(max_len_downsampled, objects.shape[0])
...
def assemble_mphoi_72_human_human_distances(data, downsampling: int = 1):
...
def assemble_mphoi_120_human_human_distances(data, downsampling: int = 1):
    mphoi_dims = np.array([1920, 1080], dtype=np.float32)
    max_len, max_num_humans = 0, 3
    h12_dists, h13_dists, h23_dists = [], [], []
    for _, _, _, _, _, h1_bb, h2_bb, h3_bb, _, _, _, _ in data:
        h1_bb = h1_bb[downsampling - 1::downsampling]
        h1_centroids = compute_centroid(h1_bb) / mphoi_dims
        h2_bb = h2_bb[downsampling - 1::downsampling]
        h2_centroids = compute_centroid(h2_bb) / mphoi_dims
        h3_bb = h3_bb[downsampling - 1::downsampling]
        h3_centroids = compute_centroid(h3_bb) / mphoi_dims
        dist12 = np.linalg.norm(h1_centroids - h2_centroids, ord=2, axis=-1)
        h12_dists.append(dist12)
        dist13 = np.linalg.norm(h1_centroids - h3_centroids, ord=2, axis=-1)
        h13_dists.append(dist13)
        dist23 = np.linalg.norm(h2_centroids - h3_centroids, ord=2, axis=-1)
        h23_dists.append(dist23)
        max_len = max(max_len, h1_bb.shape[0])
    tensor_shape = [len(h12_dists), max_len, max_num_humans, max_num_humans]
    x_hh_dists = np.full(tensor_shape, fill_value=np.nan, dtype=np.float32)
    for m, (h12_d, h13_d, h23_d) in enumerate(zip(h12_dists, h13_dists, h23_dists)):
        x_hh_dists[m, :h12_d.shape[0], 0, 1] = h12_d
        x_hh_dists[m, :h12_d.shape[0], 1, 0] = h12_d
        x_hh_dists[m, :h13_d.shape[0], 0, 2] = h13_d
        x_hh_dists[m, :h13_d.shape[0], 2, 0] = h13_d
        x_hh_dists[m, :h23_d.shape[0], 1, 2] = h23_d
        x_hh_dists[m, :h23_d.shape[0], 2, 1] = h23_d
        x_hh_dists[m, :h12_d.shape[0], 0, 0] = 0.0
        x_hh_dists[m, :h12_d.shape[0], 1, 1] = 0.0
        x_hh_dists[m, :h12_d.shape[0], 2, 2] = 0.0
    return x_hh_dists


def assemble_mphoi_72_human_object_distances(data, downsampling: int = 1):
    mphoi_dims = np.array([3840, 2160], dtype=np.float32)
    max_len, max_num_humans, max_num_objects = 0, 2, 0
    h1_dists, h2_dists = [], []
...
def assemble_mphoi_120_human_object_distances(data, downsampling: int = 1):
    mphoi_dims = np.array([1920, 1080], dtype=np.float32)
    max_len, max_num_humans, max_num_objects = 0, 3, 0
    h1_dists, h2_dists, h3_dists = [], [], []
    for _, _, _, _, _, h1_bb, h2_bb, h3_bb, obj_bbs, _, _, _, in data:
        h1_bb = h1_bb[downsampling - 1::downsampling]
        h1_centroids = compute_centroid(h1_bb) / mphoi_dims
        h2_bb = h2_bb[downsampling - 1::downsampling]
        h2_centroids = compute_centroid(h2_bb) / mphoi_dims
        ...
        h2_objs_dists = np.linalg.norm(obj_centroids - np.expand_dims(h2_centroids, axis=1), ord=2, axis=-1)
        h2_dists.append(h2_objs_dists)
        h3_objs_dists = np.linalg.norm(obj_centroids - np.expand_dims(h3_centroids, axis=1), ord=2, axis=-1)
        h3_dists.append(h3_objs_dists)
        max_len = max(max_len, h1_bb.shape[0])
        max_num_objects = max(max_num_objects, obj_bbs.shape[1])
    tensor_shape = [len(h1_dists), max_len, max_num_humans, max_num_objects]
    x_ho_dists = np.full(tensor_shape, fill_value=np.nan, dtype=np.float32)
    for m, (h1_d, h2_d, h3_d) in enumerate(zip(h1_dists, h2_dists, h3_dists)):
        x_ho_dists[m, :h1_d.shape[0], 0, :h1_d.shape[1]] = h1_d
        x_ho_dists[m, :h2_d.shape[0], 1, :h2_d.shape[1]] = h2_d
        x_ho_dists[m, :h3_d.shape[0], 2, :h3_d.shape[1]] = h3_d
    return x_ho_dists


def assemble_mphoi_72_object_object_distances(data, downsampling: int = 1):
    mphoi_dims = np.array([3840, 2160], dtype=np.float32)
    max_len, max_num_objects = 0, 0
    all_dists = []
    for _, _, _, _, _, _, obj_bbs, _, _ in data:
...
def assemble_mphoi_120_object_object_distances(data, downsampling: int = 1):
    mphoi_dims = np.array([1920, 1080], dtype=np.float32)
    max_len, max_num_objects = 0, 0
    all_dists = []
    for _, _, _, _, _, _, _, _, obj_bbs, _, _, _, in data:
        obj_bbs = obj_bbs[downsampling - 1::downsampling]
        objs_centroid = compute_centroid(obj_bbs) / mphoi_dims
        num_objects = objs_centroid.shape[1]
        dists = []
...
def select_model_data_fetcher(model_name: str, model_input_type: str, **kwargs):
    model_to_data_fetcher = {
        'GeoVisGNN': partial(gcn_fetcher, **kwargs),
    }
...
def select_model_data_feeder(model_name: str, model_input_type: str, **kwargs):
    model_to_data_forwarder = {
        'GeoVisGNN': partial(gcn_forward, **kwargs),
    }
