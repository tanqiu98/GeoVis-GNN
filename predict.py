...
def maybe_load_class_dictionaries(model_dir):
    ...
    elif dataset_name == 'mphoi120':
        dictionaries_dir = os.path.dirname(cfg.data.path)
        dictionary_path = os.path.join(dictionaries_dir, 'mphoi120_action_id_to_action_name.json')
        with open(dictionary_path, mode='r') as f:
            subactivity_id_to_name = json.load(f)
        subactivity_id_to_name = {int(k): v for k, v in subactivity_id_to_name.items()}
    else:
        dictionaries_dir = os.path.dirname(cfg.data.path)
        dictionary_path = os.path.join(dictionaries_dir, 'mphoi_72_action_id_to_action_name.json')
        with open(dictionary_path, mode='r') as f:
            subactivity_id_to_name = json.load(f)
        subactivity_id_to_name = {int(k): v for k, v in subactivity_id_to_name.items()}
...
def predict_all(args):
    ...
    if cross_validate:
        ...
        elif dataset_name == 'mphoi72':
            test_subject_ids = ['Subject45', 'Subject25', 'Subject14']
        else:
            test_subject_ids = ['Subject123', 'Subject234', 'Subject456', 'Subject567']
